"""Failure analysis of the test-week forecasts, using the predictions saved by scripts.evaluate.

Usage (from the repository root, after `python -m scripts.evaluate`):
...
from src.forecasting.plots import TRAINED_MODELS, plot_error_by_day, plot_error_by_move, plot_failure_window
...
def weekend_to_weekday_ratio(daily: pd.DataFrame) -> dict[str, float]:
    weekend = daily.index.str.startswith(WEEKEND_PREFIXES)
    return {model: float(daily.loc[weekend, model].mean() / daily.loc[~weekend, model].mean()) for model in daily}


def main() -> None:
    apply_style()
    runs = pd.read_csv(FINAL_DIR / "metrics_all_runs.csv")
    squares = list(dict.fromkeys(runs["square"]))  # evaluation order is busiest first
    predictions = {square: load_predictions(FINAL_DIR / f"predictions_{square}.csv") for square in squares}
    ...
    plot_error_by_day(
        ...
        "see failure_analysis.json).",
        config.FIGURES_DIR / "failure_error_by_day.png",
    )

    curves = {square: {m: error_by_move_size(frame, m) for m in TRAINED_MODELS} for square, frame in predictions.items()}
    plot_error_by_move(curves, config.FIGURES_DIR / "failure_error_vs_change.png")
    ...
    plot_failure_window(
        ...
        path=config.FIGURES_DIR / "failure_weekend.png",
    )
    ...
    (FINAL_DIR / "failure_analysis.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
