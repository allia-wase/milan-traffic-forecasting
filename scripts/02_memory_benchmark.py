...
Usage (from the repository root):
    python scripts/02_memory_benchmark.py --raw-dir "path/to/Dataverse" --sample-days 3
"""
import argparse
import json
...
from milan_forecasting.data.loader import (
...
from milan_forecasting.data.memory import MB, current_rss_mb, peak_rss_mb
from milan_forecasting.plotting import BASELINE, GRIDLINE, INK_MUTED, INK_PRIMARY, INK_SECONDARY, SERIES, SURFACE
...
def main() -> None:
    ...
    config.DATA_HANDLING_DIR.mkdir(parents=True, exist_ok=True)
    (config.DATA_HANDLING_DIR / "memory_benchmark.json").write_text(json.dumps(report, indent=2))
    plot_results(loading, total_ram_gb, len(files), config.DATA_HANDLING_DIR / "memory_benchmark.png")
