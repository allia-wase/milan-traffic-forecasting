...
Requires the processed matrix from `python -m scripts.build_dataset`.
...
from src.eda_plots import plot_area_series, plot_autocorrelation, plot_decomposition, plot_traffic_distribution
...
def main() -> None:
    ...
    plot_traffic_distribution(totals, top3, focus, config.FIGURES_DIR / "eda_1_traffic_distribution.png")
    ...
    plot_area_series(
        ...
        config.FIGURES_DIR / "eda_2_first_two_weeks.png",
    )

    busiest = top3[0]
    acf_values, pacf_values, band = autocorrelation(series[busiest])
    plot_autocorrelation(acf_values, pacf_values, band, busiest, config.FIGURES_DIR / "eda_3_autocorrelation.png")

    # Robust fitting keeps holidays and one-off events out of the estimated components; see
    # scripts/experiment_robust_stl.py for the comparison that motivated it.
    ...
    plot_decomposition(components, daily_anomalies, daily_bounds, ANOMALY_THRESHOLD, eval_window, busiest,
                       config.FIGURES_DIR / "eda_4_decomposition.png")
    ...
    (config.METRICS_DIR / "eda.json").write_text(json.dumps(report, indent=2, default=_json_default))
