...
from milan_forecasting.forecasting.significance import LOSSES, diebold_mariano, holm_adjusted
...
def main() -> None:
    ...
    table["holm_p_value"] = float("nan")
    for _, group in table.groupby("loss", sort=False):
        table.loc[group.index, "holm_p_value"] = holm_adjusted(group["p_value"].tolist())
    ...
    for (square, model_a, model_b), group in table.groupby(["square", "model_a", "model_b"], sort=False):
        ...
        for loss in LOSSES:
            r = by_loss.loc[loss]
            cells += [f"{r.statistic:.2f}{stars(r.holm_p_value)}", f"{r.p_value:.3g}", f"{r.holm_p_value:.3g}"]
    ...
    (config.EVALUATION_DIR / "significance.json").write_text(json.dumps({
        "family": "nine model pairs within each loss function, Holm-Bonferroni adjusted",
